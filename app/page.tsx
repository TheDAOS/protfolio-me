...
const Nav = () => (
  <nav className="fixed top-4 md:top-6 left-1/2 -translate-x-1/2 z-50 w-[95%] max-w-4xl">
    <div className="bg-white neo-border neo-shadow px-4 md:px-6 py-2 md:py-3 flex justify-between items-center">
      <span className="font-extrabold text-lg md:text-xl tracking-tighter italic">DEV.NAME</span>
      <div className="flex gap-4 md:gap-6 font-bold text-xs md:text-sm">
        <a href="#projects" className="hover:underline decoration-neo-pink decoration-2 md:decoration-4">PROJECTS</a>
        <a href="#about" className="hover:underline decoration-neo-cyan decoration-2 md:decoration-4">ABOUT</a>
        <a href="#contact" className="hover:underline decoration-neo-green decoration-2 md:decoration-4 uppercase">Hi!</a>
      </div>
    </div>
  </nav>
);

const SectionTitle = ({ children, color = "bg-neo-yellow" }: { children: React.ReactNode, color?: string }) => (
  <div className="mb-8 md:mb-12 inline-block">
    <h2 className={`text-3xl md:text-6xl font-black uppercase px-3 py-1 md:px-4 md:py-2 neo-border neo-shadow-lg ${color}`}>
      {children}
    </h2>
  </div>
);

const ProjectCard = ({ title, desc, color }: { title: string, desc: string, color: string }) => (
  <motion.div 
  ...
  >
    ...
    <div className="mt-auto flex flex-wrap gap-2">
      <span className="text-[10px] md:text-xs font-bold px-2 py-1 bg-white neo-border uppercase">React</span>
      <span className="text-[10px] md:text-xs font-bold px-2 py-1 bg-white neo-border uppercase">TS</span>
    </div>
  </motion.div>
);
  ...
  return (
    <main ref={containerRef} className="min-h-screen pt-24 md:pt-32 pb-12 md:pb-20 px-4 md:px-8">
      ...
      <div className="bg-black py-3 md:py-4 border-y-4 border-black overflow-hidden flex whitespace-nowrap mb-20 md:mb-32 -mx-4 md:-mx-8">
        <motion.div style={{ x }} className="flex gap-8 md:gap-12 text-white font-black text-2xl md:text-4xl uppercase items-center">
          {[...Array(10)].map((_, i) => (
      ...
      {/* About / Contact */}
      <section id="about" className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 mb-16 md:mb-20">
        <div className="bg-neo-green neo-border neo-shadow p-6 md:p-8 flex flex-col justify-center text-center md:text-left">
          <h3 className="text-3xl md:text-4xl font-black uppercase mb-4">Wanna Chat?</h3>
          <p className="text-base md:text-lg font-bold mb-6">
            I&apos;m always open to talking about cool projects, weird tech, or the best place to find pizza.
          </p>
          <a href="mailto:hello@example.com" className="bg-white neo-border neo-shadow px-6 py-2 font-black uppercase text-center hover:bg-black hover:text-white transition-colors inline-block w-fit mx-auto md:mx-0">
            Send an Email
          </a>
        </div>
        
        <div className="neo-border neo-shadow p-6 md:p-8 bg-white flex flex-col gap-3 md:gap-4">
          <div className="flex justify-between items-center border-b-2 md:border-b-4 border-black pb-2">
            <span className="font-black text-xl md:text-2xl uppercase underline decoration-neo-pink">GitHub</span>
            <span className="font-mono text-xs md:text-sm">@username</span>
          </div>
          <div className="flex justify-between items-center border-b-2 md:border-b-4 border-black pb-2">
            <span className="font-black text-xl md:text-2xl uppercase underline decoration-neo-cyan">Twitter</span>
            <span className="font-mono text-xs md:text-sm">@handle</span>
          </div>
          <div className="flex justify-between items-center border-b-2 md:border-b-4 border-black pb-2">
            <span className="font-black text-xl md:text-2xl uppercase underline decoration-neo-yellow">LinkedIn</span>
            <span className="font-mono text-xs md:text-sm">/in/profile</span>
          </div>
        </div>
      </section>
